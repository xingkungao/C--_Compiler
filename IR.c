			...
			case Assign: 
				{
					...
					if (code->u.assign.left->kind == Dereference)
						...
						printf("#%d", code->u.assign.left->u.val);
...
void test() {
	InterCodes* list1 = newFuncCode("main");
	list1 = codesConcat(2, list1, newUniopCode(Read, newVar(1)));
	//list1 = codesConcat(2, list1, newAssignCode(newVar(0), newVar(1)));
	//list1 = codesConcat(2, list1, newAssignCode(newVar(2), newConst(0)));
	generateIR("op1", list1);
}
